...
static esp_err_t panel_st7789_sleep(esp_lcd_panel_t *panel, bool sleep_mode);
...
static const st7789_lcd_init_cmd_t vendor_specific_init_default[] = {
    ...
    {CASET, (uint8_t[]){0x00, 0, 0, 170}, 4, 0},
...
static const st7789_lcd_init_cmd_t custom_init_cmds[] = {
    {0x11, 0, 0, 120},
    {0x36, (uint8_t[]){0x00}, 1, 0},
    {0x3A, (uint8_t[]){0x05}, 1, 0},
    {0xB2, (uint8_t[]){0x0B, 0x0B, 0x00, 0x33, 0x33}, 5, 0},
    {0xB7, (uint8_t[]){0x75}, 1, 0},
    {0xBB, (uint8_t[]){0x28}, 1, 0},
    {0xC0, (uint8_t[]){0x2C}, 1, 0},
    {0xC2, (uint8_t[]){0x01}, 1, 0},
    {0xC3, (uint8_t[]){0x1F}, 1, 0},
    {0xC6, (uint8_t[]){0x13}, 1, 0},
    {0xD0, (uint8_t[]){0xA7}, 1, 0},
    {0xD0, (uint8_t[]){0xA4, 0xA1}, 2, 0},
    {0xD6, (uint8_t[]){0xA1}, 1, 0},
    {0xE0, (uint8_t[]){0xF0, 0x05, 0x0A, 0x06, 0x06, 0x03, 0x2B, 0x32, 0x43, 0x36, 0x11, 0x10, 0x2B, 0x32}, 14, 0},
    {0xE1, (uint8_t[]){0xF0, 0x08, 0x0C, 0x0B, 0x09, 0x24, 0x2B, 0x22, 0x43, 0x38, 0x15, 0x16, 0x2F, 0x37}, 14, 0},
    {0x21, 0, 0, 0},
    {0x29, 0, 0, 0},
    {0x2C, 0, 0, 0},
};
...
static esp_err_t panel_st7789_init(esp_lcd_panel_t *panel)
{
    ...
    else
    {
        ...
        init_cmds_size = sizeof(custom_init_cmds) /
...
static esp_err_t panel_st7789_sleep(esp_lcd_panel_t *panel, bool sleep_mode)
{
    ...
    if (sleep_mode) {
        // Enter sleep mode
        ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, SLPIN, NULL, 0), TAG, "send command failed");
    } else {
        // Exit sleep mode
        ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, SLPOUT, NULL, 0), TAG, "send command failed");
        // According to ST7789 datasheet, it requires at least 120ms after sending SLPOUT before sending another command
